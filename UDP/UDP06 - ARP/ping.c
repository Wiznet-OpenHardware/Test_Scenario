...
uint8_t ping_request(uint8_t s, uint8_t *addr){
  uint16_t i;
	...
	PingRequest.ID = htons_ipraw(RandomID++);	       // set ping-request's ID to random integer value
	PingRequest.SeqNum =htons_ipraw(RandomSeqNum++);// set ping-request's sequence number to ramdom integer value
	...
	PingRequest.CheckSum = htons_ipraw(checksum((uint8_t*)&PingRequest,sizeof(PingRequest)));  // Calculate checksum
	...
	if(sendto(s,(uint8_t *)&PingRequest,sizeof(PingRequest),addr,3000)==0){  // Send Ping-Request to the specified peer.
		  ...
		  printf( " ID:%x  SeqNum:%x CheckSum:%x\r\n",   htons_ipraw(PingRequest.ID),  htons_ipraw(PingRequest.SeqNum),  htons_ipraw(PingRequest.CheckSum)) ;
	}
	return 0;
} // ping request
		...
		/* receive data from a destination */
	  	len = recvfrom(s, (uint8_t *)data_buf,rlen,addr,&port);
			if(data_buf[0] == PING_REPLY) {
				...
				if(tmp_checksum != 0xffff)
					...
					/*  Output the Destination IP and the size of the Ping Reply Message*/
				    	printf("Reply from %d.%d.%d.%d  ID:%x SeqNum:%x  :data size %d bytes\r\n",
						  (addr[0]),  (addr[1]),  (addr[2]),  (addr[3]),  htons_ipraw(PingReply.ID),  htons_ipraw(PingReply.SeqNum),  (rlen+6) );
				    	printf("\r\n");
				...
				PingReply.CheckSum = htons_ipraw(checksum(&PingReply,len));

						if(tmp_checksum != PingReply.CheckSum){
							printf( " \n CheckSum is in correct %x shold be %x \n",   (tmp_checksum),  htons_ipraw(PingReply.CheckSum)) ;
						}else{
							//printf( "\r\n Checksum is correct  \r\n") ;					
						}
...
uint16_t htons_ipraw( uint16_t hostshort)
{
#if 1
  //#ifdef LITTLE_ENDIAN
	uint16_t netshort=0;
